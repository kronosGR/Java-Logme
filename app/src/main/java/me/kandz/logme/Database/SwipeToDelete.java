...
public class SwipeToDelete  extends ItemTouchHelper.SimpleCallback {
    ...
    public SwipeToDelete(ExtrasAdapter adapter) {
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if (dX>0){
            background.setBounds(itemView.getLeft(),
                    itemView.getTop(),
                    itemView.getLabelFor() + ((int)dX) + backOffset,
                    itemView.getBottom());
        } else if (dX < 0){
            background.setBounds(itemView.getRight() + ((int) dX) - backOffset,
        ...
        if (dX > 0) { // Swiping to the right
            ...
            background.setBounds(itemView.getLeft(), itemView.getTop(),
                    itemView.getLeft() + ((int) dX) + backOffset,
                    itemView.getBottom());
